...
for i in range(1):
    ...
    ts = np.linspace(0, 15*np.pi, 200)
    ts_ana = np.linspace(0, 15*np.pi, 50)
    ...
    solution_ho, loss_normal = ode.solve(ode=oscillator, condition=init_val_ho, net=net_ho,
                           max_epochs=3000,t_min=0.0, t_max=2*np.pi)
    ...
    plt.savefig('PlotsMarch11/FreqGuesses/MasternodeOmega{}Period.png'.format(omega))
    ...
    plt.savefig('PlotsMarch11/FreqGuesses/MasternodeLossOmega{}Period.png'.format(omega))
