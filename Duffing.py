...
duffing = lambda x, t: diff(x, t, order=2) + delta*diff(x, t) + alpha*x + beta*x**3 - gamma*torch.cos(omega*t)
init_val_ho = ode.IVP(t_0=0.0, x_0=0.0, x_0_prime=1.0)

# let's find ground truth first
def vdp(t, y):
    x, x_dot = y
    return [x_dot, -alpha*x - beta*x**3 - delta*x_dot + gamma*np.cos(omega*t)]
...
train_gen = ode.ExampleGenerator(size=200,  t_min=0.0, t_max=10, method='equally-spaced-noisy')
valid_gen = ode.ExampleGenerator(size=100, t_min=0.0, t_max=10, method='uniform')
net_ho = FCNN(
    n_hidden_layers=3, n_hidden_units=32, actv=nn.Tanh
)
solution_ho, loss_normal = ode.solve(ode=duffing, condition=init_val_ho, net=net_ho,
                       max_epochs=10000,t_min=0.0, t_max=10,
...
net_ho = FCNN_masternode(n_hidden_layers=3, n_hidden_units=32, actv=nn.LeakyReLU)
train_gen = ode.ExampleGenerator(size=200,  t_min=0.0, t_max=10, method='equally-spaced-noisy')
valid_gen = ode.ExampleGenerator(size=200, t_min=0.0, t_max=10, method='uniform')
init_val_ho = ode_masternode.IVP(t_0=0.0, x_0=0.0, x_0_prime=1.0)
# solve the ODE
solution_ho, loss_per_enforce = ode_masternode.solve(
    ode=duffing, condition=init_val_ho,  max_epochs=10000,t_min=0.0, t_max=10,
...
plt.title('Solutions to the Duffing equation', fontsize = 18)
plt.legend(fontsize = 18)
plt.savefig('PlotsMarch19/DuffingAna.png')
plt.show()
...
plt.legend(fontsize = 18)
plt.savefig('PlotsMarch19/DuffingLoss.png')
